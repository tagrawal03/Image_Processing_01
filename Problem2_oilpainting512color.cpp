...
/*Code to implement oil painting with 512 colors per pixel*/
bool Image::oil_painting()
{
   ...
   for (int i = 0; i < 256; i++)
   {
      ...
      /*Calculate threshold values- Choose threshold(x axis) where 25%, 50%, 75% 100%
       pixels lie(y axis)*/
      //Red
      if (cd_histR[i] > (0.125*r*height*width))
      {
         //store y axis intensity value as threshold
         binR[r] = i-1;
         r = r+1;
      }
      else if (cd_histR[i] == (0.125*r*height*width))
      {
         //store y axis intensity value as threshold
         binR[r] = i;
         r = r+1;
     ...
     //Green
      if (cd_histG[i] > (0.125*g*height*width))
      {
         //store y axis intensity value as threshold
         binG[g] = i-1;
         g = g+1;
      }
      else if (cd_histG[i] == (0.125*g*height*width))
      {
         //store y axis intensity value as threshold
         binG[g] = i;
         g = g+1;
      ...
      //Blue
      if (cd_histB[i] > (0.125*b*height*width))
      {
         //store y axis intensity value as threshold
         binB[b] = i-1;
         b = b+1;
      }
      else if (cd_histB[i] == (0.125*b*height*width))
      {
         //store y axis intensity value as threshold
         binB[b] = i;
         b = b+1;
      ...
      int meanR[8]= {0}, meanG[8]={0}, meanB[8]={0};
      for(int j =1; j<9; j++)
      {
